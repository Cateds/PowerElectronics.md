...
> 压缩机电机的转速和相应的压缩机输出会调整至与建筑供暖/制冷负荷相匹配，进而消除压缩机的启停循环以及导致的压缩机输出损失。
>
...
> 可以精确加热工件的指定区域，感应电流大小根据趋肤深度 $x$ 指数递减，具体的公式为
>
> $$
> I(x) = I_0 \exp(-\frac{x}{\delta})
> $$
>
> 其中 $I_0$ 是工件表面处的电流，$\delta$ 是电流衰减到 $1/e$ 的深度，称为趋肤深度 (Skin Depth)，其计算公式为
>
> $$
...
> 焊机的电压-电流特性取决于采用的焊接工艺类型。典型的额定电压和电流分别为 $50V$ 和 $500A$
>
...
电枢电压 (Armature Voltage) 是电动机承载工作电流并产生转矩的绕组的两端电压，大小为
...
- **内环：电流环，红色**
  - 用来限制电流，**保护**电机不被过流摧毁
  - 同时因为转矩和电流有正比例关系，电流环也可以用来实现**转矩控制**
...
使用鼠笼式转子的 **感应电机 (Induction Motor)** 因为低成本和结构坚固而成为工业领域的主要设备。下图中左图是滑环式转子 (Slip Ring Rotor)，右图是鼠笼式转子 (Squirrel Cage Rotor)。
...
![induction motor drive](lec13.assets/image-13.png)
