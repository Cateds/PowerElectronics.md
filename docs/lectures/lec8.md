...
因为电感电压的积分等于电感的磁链的变化关系，我们可以得到在一个周期内电感器两端的电压积分为零。这是稳态工作时的条件。也就是说
...
类似地，我们也可以通过每个周期结束后通过电感的电流大小应该保持不变来分析输入输出关系。具体的等量关系是
...
类似地，假设 Boost 变换器是一个理想电路，也就是输入输出功率相等 $P_{in} = P_{out}$，可以得到
...
Buck-Boost 变换器同时适用于产生高于或者低于输入电压的输出电压。仍然工作在连续模式。

- 当开关占空比为 0 时
  - 此时电源的输入端和输出端之间没有连接。因此，电源一直对外提供 $0V$ 的输出。
- 当开关闭合时
...
同样地，假设 Buck-Boost 变换器是一个理想电路，也就是输入输出功率相等 $P_{in} = P_{out}$，可以得到
...
对于理想情况下的三种 DC-DC 开关变换器，它们的电压关系和电流关系如下所示：
