...
桥式电路与全波整流电路之间的关系时相同的（因为理想情况下输出是一致的）。因此，有 $V_{DC} \approx 0.9 V_{AC\ RMS}$。
...
这里的 $V_{pp}$ 是输出纹波电压的峰峰值，$V_{p(sec)}$ 是变压器次级电压的峰值。此时，输出的直流电压为
...
较大的电流会导致额外的发热，计算是 $I^2t$，被称为**允通能量 (Let-throught Energy)**，其与电流的平方成正比，与时间成正比，因此 $10A$ 持续 $10ms$ 和 $100A$ 持续 $1ms$ 的发热损耗的比例是 $1:10$
...
变压器只包含电器损耗，也就是铁损 (涡流损耗和磁滞损耗) 和铜损 (线圈电阻引起的损耗)，因此效率较高，通常在 $95\% \sim 98.5\%$
...
2. **全波桥式整流器比中间抽头更能有效利用变压器绕组**
   ...
   桥式整流器中每个二极管的反向电压时中间抽头的一半，因为对于桥式电路有两个二极管分担电压，也就是
...
左图的形式被称为横流限制 (Constant Current Limiting)，调节器保持输出电流恒定的同时降低输出电压。这种方法的问题是降低输出电压会导致压差增大，进而导致功率损耗增大。
