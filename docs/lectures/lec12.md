# Lec.12 AC-DC 变换器 - III: 三桥臂逆变器

> **_AC-DC Converters (Inverters)_**
>
> Lecture @ 2026-5-21
...
最常用的三相逆变器电路由三个桥臂组成，每相一个桥臂。每个逆变器的结构与基本的半桥逆变器的桥臂类似。每个桥臂的输出电压 $v_{AN}$ 只取决于直流电压 $V_{d}$ 和开关状态
...
| --- | ---------- | ---------- | ---------- | -------- | -------- | -------- |
| 1   | **_闭合_** | 断开       | **_闭合_** | $V_D$    | $-V_D$   | $0$      |
| 2   | **_闭合_** | 断开       | 断开       | $V_D$    | $0$      | $-V_D$   |
| 3   | **_闭合_** | **_闭合_** | 断开       | $0$      | $V_D$    | $-V_D$   |
| 4   | 断开       | **_闭合_** | 断开       | $-V_D$   | $V_D$    | $0$      |
| 5   | 断开       | **_闭合_** | **_闭合_** | $-V_D$   | $0$      | $V_D$    |
| 6   | 断开       | 断开       | **_闭合_** | $0$      | $-V_D$   | $V_D$    |
...
三桥臂逆变器的 PWM 输出和单相不同，控制信号由三个，是三路相通幅度、频率，相位差是 $\frac{2\pi}{3}$ 的正弦波信号，计作 $v_{control,A}$、$v_{control,B}$ 和 $v_{control,C}$，频率为期望的基波频率 $f_1$。
...
- 当 $m_f$ 为 3 的倍数并且时奇数时，谐波成分在线电压中被抵消了
...
其中，A 相和 B 相的三次谐波在频率、相位、幅度上时完全相等的，在计算电压的差值时就被抵消了。

### 实际的逆变器

半桥逆变器中使用的控制硬件可以应用于三相逆变器的每相桥臂，以实现开关控制的作用。同样的，可以使用类似的记录来创造死区时间，避免方波和 PWM 开关中的直通问题。
